...
import com.startdis.comm.jdbc.handler.*;
import com.startdis.comm.jdbc.interceptor.FieldFillInterceptor;
...
public class MybatisPlusConfig {
    ...
    public MybatisPlusInterceptor mybatisPlusInterceptor() {
        ...
        // 多租户（集团）拦截器
        Boolean groupTenantEnable = tenantConfig.getGroupTenant().getEnable();
        if (groupTenantEnable){
            TenantLineInnerInterceptor tenantSqlParser = new TenantLineInnerInterceptor();
            tenantSqlParser.setTenantLineHandler(getGroupTenantInterceptor());
    ...
    @Bean
    public GroupTenantHandler getGroupTenantInterceptor() {
        return new GroupTenantHandler(tenantConfig);
    }
