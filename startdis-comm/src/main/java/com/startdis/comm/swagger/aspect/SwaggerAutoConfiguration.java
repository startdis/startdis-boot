...
public class SwaggerAutoConfiguration {
    ...
    private ApiInfo groupApiInfo() {
        return new ApiInfoBuilder()
                .title("Startdis 启迪数转平台")
                .description("<div style='font-size:14px;color:red;'>Startdis RESTful APIs</div>")
                .termsOfServiceUrl("http://www.startdis.com/")
                .contact(new Contact("云顶数字","https://yundingshuzi.com","startdis@yundingshuzi.cn"))
                .version("1.0")
                .build();
    }
    ...
    private List<RequestParameter> setHeaderParam() {
        ...
        authToken.name(HeaderConstant.X_AUTH_TOKEN).description("公共-用户鉴权Token").in(ParameterType.HEADER).required(false).build();
